...
| **Enter SPAM / auto-repeat setelah scan** (POS dapat Enter berulang, tombol Enter fisik ikut mati) | Enter tertinggal "tertekan" karena keyup-nya hilang saat `press()` dikirim terlalu cepat → OS auto-repeat. Agent versi terbaru menekan Enter **eksplisit** (keyDown → tahan ~80 ms → keyUp) + reset state keyboard sebelum/sesudah tiap scan. **Update agent** (curl install di atas) lalu coba lagi. **Perbaiki sekarang**: tekan Shift 5× atau buka On-Screen Keyboard (`osk`) lalu klik sekali — atau restart. |
