...
| **Enter SPAM / auto-repeat setelah scan** (POS dapat Enter berulang, tombol Enter fisik ikut mati; Win+Enter tetap jalan) | Enter tertinggal "tertekan" karena keyup-nya hilang di jalur input berbasis VK code (bisa terjadi pada sebagian driver keyboard/IME). **v2.2+**: di Windows Enter dikirim via **scancode langsung** (SendInput + `KEYEVENTF_SCANCODE`) **dan agent mengecek sendiri state Enter via `GetAsyncKeyState` setelah tiap scan** — bila masih tertekan, keyup dikirim ulang otomatis; di banner startup juga ada cek (log "Enter terdeteksi terlepas ✅" atau peringatan remapper). **Cek versi**: banner startup harus `v2.2` — kalau belum, update agent (curl install di atas). **Perbaiki sekarang**: tekan Shift 5× atau klik sekali tombol di On-Screen Keyboard (`osk`) — atau restart. |
