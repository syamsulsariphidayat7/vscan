...
| **Enter SPAM / auto-repeat setelah scan** (POS dapat Enter berulang, tombol Enter fisik ikut mati; Win+Enter tetap jalan) | Enter tertinggal "tertekan" karena keyup-nya hilang di jalur input berbasis VK code (bisa terjadi pada sebagian driver keyboard/IME). **v2.1+**: di Windows Enter dikirim via **scancode langsung** (SendInput + `KEYEVENTF_SCANCODE` — bebas layout/IME, persis keyboard fisik) + reset keyboard sebelum/sesudah tiap scan. **Cek versi**: banner startup harus menampilkan `v2.1` — kalau belum, update agent (curl install di atas; `agent.env` dipertahankan). **Perbaiki sekarang**: tekan Shift 5× atau klik sekali tombol di On-Screen Keyboard (`osk`) — atau restart. |
